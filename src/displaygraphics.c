...
#include <string.h>
#include <stdio.h>
#include "../inc/img/startupGraph.h"
...
static void GRAPHICS_DrawFenceVoltage(int32_t xoffset, int32_t yoffset, uint32_t vFence);
 ...
 *        pixels
 ******************************************************************************/
void GRAPHICS_Startup()
{
  GLIB_clear(&glibContext);
  GLIB_drawBitmap(&glibContext, 0, 0,128, 128, sheep);
  DMD_updateDisplay();
}
 ...
 ******************************************************************************/
void GRAPHICS_Draw(int32_t tempData, uint32_t rhData, uint32_t vFence)
{
  GLIB_clear(&glibContext);
    ...
    GRAPHICS_DrawHumidity(64 - 17, 3, rhData);
    GRAPHICS_DrawFenceVoltage(127 - 40, 3, vFence);
...
static void GRAPHICS_DrawFenceVoltage(int32_t xoffset, int32_t yoffset, uint32_t vFence)
{
  ...
  GRAPHICS_CreateString(string, vFence * 3.05);
  GLIB_drawString(&glibContext, string, strlen(string), xoffset, yoffset, 0);

  GRAPHICS_DrawThermometer(xoffset + 15, yoffset + 17, 12, vFence * 12/4096, 'V');
}
